          ...
          <Reveal>
            <div className="card card-top-glow p-7 sm:p-9">
              <p className="text-[15px] leading-9 text-ink-800 sm:text-base">
                تقدم <span className="font-bold text-ink-950">{BRAND.name}</span> حلولًا وخيارات
              ...
              <div className="mt-7">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-2xl border border-brand-900/15 bg-brand-50/70 px-5 py-3 text-sm font-semibold text-ink-950 transition hover:border-brand-400/50 hover:bg-brand-500/10"
                >
                  تعرف على خدماتنا
            ...
            {points.map((p, i) => (
              <Reveal key={p.title} delay={i * 90}>
                <div className="card card-hover h-full p-6">
                  <span className="grid h-11 w-11 place-items-center rounded-2xl border border-brand-400/25 bg-brand-500/[0.12] text-brand-600">
                    <Icon name={p.icon} size={20} />
                  </span>
                  <h3 className="mt-4 text-[15px] font-bold">{p.title}</h3>
                  <p className="mt-2 text-[13px] leading-6 text-ink-700">{p.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
